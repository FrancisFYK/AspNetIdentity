    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                //    {
                //        var user = new ApplicationUser { UserName = loginInfo.DefaultUserName, Email = "123@email.com" };
                //        var claimDict = loginInfo.ExternalIdentity.Claims.ToDictionary(z => z.Type, z => z.Value);
                //        var createIdentityResult = await UserManager.CreateAsync(user);
                //        if (createIdentityResult.Succeeded)
                //        {
                //            createIdentityResult = await UserManager.AddLoginAsync(user.Id, loginInfo.Login);
                //            if (createIdentityResult.Succeeded)
                //            {
                //                await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                //                return RedirectToAction("SendCode", new { ReturnUrl = returnUrl, RememberMe = false });
